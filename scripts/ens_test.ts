...
import { namehash, bytesToHex, zeroAddress, hexToBytes, keccak256, toBytes, encodeFunctionData  } from "viem";
...
async function main() {
  ...
  // 2) Query rent price
  const priceData = await controller.read.rentPrice([LABEL, DURATION]);
  console.log("rentPrice base/premium:", priceData.base.toString(), priceData.premium.toString());
  ...
  try {
    ...
    console.log("✅ simulateContract succeeded (no revert)");
  } catch (err: any) {
    console.error("❌ simulateContract reverted:", err.cause?.shortMessage || err.message);
    process.exit(1);
  }
  ...
  const CRE8OR_NODE = "0xdb16739af6cfc75c90f34d005d9cd5bf924767f495f495a3ff96537a5bde11e6";
